...
.PHONY: kafka-ims-server-rpm kafka-ims-rest-server-rpm kafka-ims-client-rpm kafka-ims-rest-client-rpm
...
kafka-ims-rest-client-rpm: ${HOME}/rpmbuild/RPMS/noarch/kafka-ims-rest-client-${version}-${release}.noarch.rpm
${HOME}/rpmbuild/RPMS/noarch/kafka-ims-rest-client-${version}-${release}.noarch.rpm: kafka-ims—rest-client-rpmbuild
	docker run --rm kafka-ims-rest-client-rpmbuild tar cf - rpmbuild/RPMS/noarch/kafka-ims-rest-client-${version}-${release}.noarch.rpm | tar xvf - -C /Users/nex37045

kafka-ims—rest-client-rpmbuild: \
  kafka-ims-rest-client-rpmbuild.spec \
  Dockerfile-rest-client-rpmbuild
	DOCKER_BUILDKIT=1 docker build -t kafka-ims-rest-client-rpmbuild --build-arg version=${version} --build-arg release=${release} -f Dockerfile-rest-client-rpmbuild --progress=plain .

server-rpm: kafka-ims-server-rpm kafka-ims-rest-server-rpm 

client-rpm: kafka-ims-client-rpm kafka-ims-rest-client-rpm
...
deploy: rpm
	...
	jfrog rt u --server-id=corp ${HOME}/rpmbuild/RPMS/noarch/kafka-ims-rest-client-${version}-${release}.noarch.rpm rpm-dim-kafka-ims-release-local/7/dev/

clean:
	docker rmi -f kafka-ims-server-rpmbuild kafka-ims-rest-server-rpmbuild kafka-ims-client-rpmbuild kafka-ims-rest-client-rpmbuild || exit 0
