    ...
    public void filter(ContainerRequestContext containerRequestContext) throws IOException {
        ...
        if (containerRequestContext.getSecurityContext() != null) {
            final String resourceType =
                OAuthRestProxyUtil.getResourceType(containerRequestContext, resourceInfo);
            log.debug("ResourceType: " + resourceType);
            final IMSBearerTokenJwt principal =
                OAuthRestProxyUtil.getBearerInformation(containerRequestContext, resourceInfo);
            log.debug("Principal: " + principal.toString());
            final KafkaRestContext context =
                this.getKafkaRestContext(resourceType, principal, containerRequestContext);
            log.debug("Context: " + context.toString());
            KafkaRestContextProvider.setCurrentContext(context);
        }
    }

    private KafkaRestContext getKafkaRestContext(
        final String resourceType,
        final IMSBearerTokenJwt principal,
    ...
    throws IMSRestException {
        ...
        if (principal instanceof IMSBearerTokenJwt) {
            ...
            try {
                ...
                throw new IMSRestException(
                    IMSRestException.REST_CONFIGURATION_ERROR_CODE,
                    IMSRestException.REST_CONFIGURATION_ERROR_MSG + e.getMessage());
            }
            log.debug("Get context using Factory");
            context =
                KafkaOAuthRestContextFactory.getInstance()
                .getContext(principal, bearerTokenKafkaRestConfig, resourceType, true);
        } else {
            ...
            throw new IMSRestException(
                IMSRestException.BEARER_IS_NOT_INSTANCE_IMS_JWT_CODE,
                IMSRestException.BEARER_IS_NOT_INSTANCE_IMS_JWT_MSG);
        }
        log.debug("context: " + context.toString());
        return context;
    }
